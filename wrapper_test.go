...
func BenchmarkWrapper_Append(b *testing.B) {
	...
	commonSliceAppendBenchmark(b, r, EmptySlice[int](0, 0))
}

func BenchmarkWrapper_Prepend(b *testing.B) {
	r := rand.New(rand.NewSource(time.Now().Unix()))
	commonSlicePrependBenchmark(b, r, EmptySlice[int](0, 0))
}

func BenchmarkWrapper_Erase(b *testing.B) {
	r := rand.New(rand.NewSource(time.Now().Unix()))
	commonSliceEraseBenchmark(b, r, EmptySlice[int](0, 0))
}

func BenchmarkWrapper_Index(b *testing.B) {
	r := rand.New(rand.NewSource(time.Now().Unix()))
	commonSliceIndexBenchmark(b, r, EmptySlice[int](0, 0))
}

func BenchmarkWrapper_Iter(b *testing.B) {
	r := rand.New(rand.NewSource(time.Now().Unix()))
	commonSliceIterBenchmark(b, r, EmptySlice[int](0, 0))
}
